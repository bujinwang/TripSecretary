...
class JapanDataValidator {
  ...
  static validatePassportData(passport: PassportData | null | undefined): ValidationResult {
    ...
    requiredFields.forEach(field => {
      ...
      if (!hasValue) {
        errors.push(`Passport ${String(field)} is required`);
      }
    });
  ...
  static validatePersonalInfo(personalInfo: PersonalInfoData | null | undefined): ValidationResult {
    const errors: string[] = [];
    const requiredFields: Array<keyof PersonalInfoData> = ['occupation', 'cityOfResidence', 'residentCountry', 'phoneNumber', 'email', 'gender'];
    const fieldStatus: FieldStatus = {};
    ...
    requiredFields.forEach(field => {
      ...
      if (!hasValue) {
        errors.push(`Personal info ${String(field)} is required`);
      }
    });
  ...
  static validateTravelInfo(travelInfo: TravelInfoData | null | undefined): ValidationResult {
    ...
    requiredFields.forEach(field => {
      ...
      if (!hasValue) {
        errors.push(`Travel info ${String(field)} is required`);
      }
    });
    ...
    if (travelInfo) {
      ...
      if (travelInfo.lengthOfStay) {
        const days = parseInt(String(travelInfo.lengthOfStay), 10);
      ...
      if (travelInfo.travelPurpose) {
        ...
        if (!validPurposes.includes(normalizedPurpose)) {
          ...
          (travelInfo as { travelPurpose: string }).travelPurpose = normalizedPurpose;
        }
      }
    }
  ...
  static async validateComplete(userId: UserId): Promise<CompleteValidationResult> {
    try {
      ...
      console.error('Failed to validate complete Japan data:', errorMessage);
      return {
        isValid: false,
        isComplete: false,
