...
class CacheManager {
   ...
   * @constant
   */
  static readonly CACHE_TTL: number = 5 * 60 * 1000; // 5 minutes
   ...
   */
  static updateTimestamp(cacheKey: string): void {
   ...
   */
  static invalidate(dataType: string, userId: string): void {
   ...
   */
  static refreshUser(userId: string): void {
   ...
   */
  static recordHit(dataType: string, userId: string): void {
   ...
   */
  static recordMiss(dataType: string, userId: string): void {
   ...
   */
  static get<T = any>(dataType: string, userId: string): T | undefined {
    const cacheKey = `${dataType}_${userId}`;
    ...
    // Get data from cache
    const data = CacheStore.get<T>(dataType, userId);
    if (data) {
   ...
   */
  static set<T = any>(dataType: string, userId: string, data: T): void {
   ...
   * }
   */
  static has(dataType: string, userId: string): boolean {
    const cacheKey = `${dataType}_${userId}`;
