...
class PassportOperations {
   ...
   */
  static async getPassport(userId, triggerDataChangeEvent) {
    try {
  ...
  static async getPrimaryPassport(userId) {
    try {
      ...
      // Check cache first
      if (CacheManager.isValid(cacheKey)) {
        const cached = CacheStore.cache.primaryPassport?.get(userId);
        if (cached) {
      ...
      if (primaryPassport) {
        if (!CacheStore.cache.primaryPassport) {
          CacheStore.cache.primaryPassport = new Map();
        }
        CacheStore.cache.primaryPassport.set(userId, primaryPassport);
        CacheManager.updateTimestamp(cacheKey);
      }
  ...
  static async getAllPassports(userId) {
    try {
      ...
      // Check cache first
      if (CacheManager.isValid(cacheKey)) {
        const cached = CacheStore.cache.allPassports?.get(userId);
        if (cached) {
      ...
      if (!CacheStore.cache.allPassports) {
        CacheStore.cache.allPassports = new Map();
      }
      CacheStore.cache.allPassports.set(userId, passports);
      CacheManager.updateTimestamp(cacheKey);
  ...
  static async getPassportCountries(passportId) {
    try {
      ...
      // Check cache first
      if (CacheManager.isValid(cacheKey)) {
        const cached = CacheStore.cache.passportCountries?.get(passportId);
        if (cached) {
      ...
      if (!CacheStore.cache.passportCountries) {
        CacheStore.cache.passportCountries = new Map();
      }
      CacheStore.cache.passportCountries.set(passportId, passportCountries);
      CacheManager.updateTimestamp(cacheKey);
   ...
   */
  static async savePassport(passportData, userId, options = {}, triggerDataChangeEvent) {
    if (!userId) {
   ...
   */
  static async updatePassport(passportId, updates, options = {}, triggerDataChangeEvent) {
    try {
