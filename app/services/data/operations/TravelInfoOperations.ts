...
type GetEntryInfoByDestination = (destination: string) => Promise<any>;
...
class TravelInfoOperations {
   ...
   static async updateTravelInfo(
     ...
     try {
       ...
       if (!existing) {
         ...
         if (createdArrivalDate && getEntryInfoByDestination) {
           await TravelInfoOperations.handleArrivalDateChange(userId, destination, null, createdArrivalDate, getEntryInfoByDestination);
         }
       ...
       if (arrivalDateChanged && getEntryInfoByDestination) {
   ...
   */
  static async handleArrivalDateChange(
    userId: UserId,
    destination: string,
