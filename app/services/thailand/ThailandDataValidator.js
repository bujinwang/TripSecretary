...
class ThailandDataValidator {
  ...
  static validatePersonalInfo(personalInfo) {
    ...
    const requiredFields = ['occupation', 'phoneNumber', 'email'];
    const fieldStatus = {};
  ...
  static validateTravelInfo(travelInfo) {
    ...
    if (travelInfo) {
      ...
      dateFields.forEach(({ field, name }) => {
        if (travelInfo[field] && !this.isValidDate(travelInfo[field])) {
  ...
  static async validateComplete(userId) {
    try {
      ...
      console.error('Failed to validate complete Thailand data:', error);
      return {
        isValid: false,
        isComplete: false,
   ...
   */
  static getMinimumFundAmount(currency) {
   ...
   */
  static checkSubmissionWindow(arrivalDate) {
