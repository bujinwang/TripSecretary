...
travelInfoUpdates.lengthOfStay = lengthOfStay;
      ...
      console.error('Error saving US travel info data:', error);
      throw error;
    }
  }, [
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [passportData, personalInfo, travelInfo, fundItems] = await Promise.all([
        ...
        UserDataService.getTravelInfo(userId, 'us').catch(() => null),
      ...
      console.error('Error loading US travel info data:', error);
      Alert.alert(
        t('us.travelInfo.errors.loadingFailed', { defaultValue: 'Loading Failed' }),
        t('us.travelInfo.errors.loadingFailedMessage', { defaultValue: 'Failed to load travel info' }),
        [{ text: 'OK' }]
      );
    } finally {
      setIsLoading(false);
