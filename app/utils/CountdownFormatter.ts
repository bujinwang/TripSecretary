...
interface CountdownContextResult extends CountdownResult {
  contextMessage: string;
  context: string;
}

interface CountdownColorHints {
  primary: string;
  background: string;
  text: string;
  border: string;
...
interface CountdownMultipleResult {
  compact: CountdownResult;
  detailed: CountdownResult;
  simple: CountdownResult;
  minimal: CountdownResult;
}

type LiveCountdownStopper = () => void;

class CountdownFormatter {
  /**
   * Format time remaining for display
   * @param milliseconds - Time remaining in milliseconds
   * @param locale - Locale code ('zh', 'en', 'es', etc.)
   * @param options - Formatting options
   * @returns Formatted countdown information
   */
   ...
   * @param totalHours - Total hours remaining
   * @returns Urgency information
   */
  static calculateUrgency(totalHours: number): UrgencyInfo {
    if (totalHours <= 0) {
   ...
   * @param parts - Array of formatted time parts
   * @param components - Time components
   * @param options - Formatting options
   * @returns Chinese-optimized time display
   */
  static formatChineseTime(
    parts: string[],
    components: CountdownComponents,
    options: FormatDisplayOptions = {}
  ): string {
   ...
   * @param value - Value for the unit
   * @param locale - Locale code
   * @param shortFormat - Use short format
   * @returns Formatted time unit
   */
  static formatTimeUnit(
    unit: CountdownUnit,
    value: number,
    locale: string,
   ...
   * @returns Correct form based on value
   */
   ...
   * @param parts - Array of formatted time parts
   * @param locale - Locale code
   * @returns Joined time string
   */
  static joinTimeParts(parts: string[], locale: string): string {
    if (parts.length === 0) {
      return '';
    }
    if (parts.length === 1) {
   ...
   * @param milliseconds - Time remaining in milliseconds
   * @param locale - Locale code
   * @param context - Context ('until_deadline', 'until_arrival', 'until_window_opens')
   * @param options - Formatting options
   * @returns Formatted countdown with context
   */
  static formatWithContext(
    milliseconds: number,
    locale: string = 'zh',
    context: string = 'until_deadline',
    options: CountdownFormatOptions = {}
  ): CountdownContextResult {
    const countdown = this.formatTimeRemaining(milliseconds, locale, options);
    
    if (countdown.isEmpty) {
      return {
        ...countdown,
        contextMessage: countdown.display,
        context
      };
    }
   ...
   * @returns Color information for UI
   */
  static getColorHints(urgencyLevel: string): CountdownColorHints {
    const colorMap = {
      expired: {
        primary: '#FF4444',
        background: '#FFEBEE',
    ...
    return (colorMap as Record<string, CountdownColorHints>)[urgencyLevel] || colorMap.low;
  }
   ...
   * @param locale - Locale code
   * @param options - Formatting options
   * @returns Function to stop the countdown
   */
  static createLiveCountdown(
    targetDate: Date,
    callback: (result: CountdownResult) => void,
    locale: string = 'zh',
    options: CountdownFormatOptions & { updateInterval?: number } = {}
  ): LiveCountdownStopper {
   ...
   */
  static formatMultiple(milliseconds: number, locale: string = 'zh'): CountdownMultipleResult {
    return {
