...
const SINGAPORE_ENTRY_STEPS = singaporeGuideConfig.steps;
...
const SingaporeEntryGuideScreen = ({ navigation, route }) => {
  ...
  const currentStep = SINGAPORE_ENTRY_STEPS[currentStepIndex];
  const progress = ((currentStepIndex + 1) / SINGAPORE_ENTRY_STEPS.length) * 100;
  ...
  const handleNextStep = () => {
    if (currentStepIndex < SINGAPORE_ENTRY_STEPS.length - 1) {
  ...
  const handleStepPress = (stepIndex) => {
    ...
    if (stepIndex <= currentStepIndex || completedSteps.has(SINGAPORE_ENTRY_STEPS[stepIndex].id)) {
      setCurrentStepIndex(stepIndex);
    }
  };

  const getStepStatus = (stepIndex) => {
    const step = SINGAPORE_ENTRY_STEPS[stepIndex];
  ...
  const renderStepIndicator = () => {
    return (
      <View style={styles.stepIndicatorContainer}>
        ...
        >
          {SINGAPORE_ENTRY_STEPS.map((step, index) => {
  ...
  const handleOpenEntryPack = () => {
    if (!completionData) {
      Alert.alert(
        ...
        t('singapore.entryGuide.missingData', { defaultValue: '请先在入境资料中填写必需信息，再尝试打开通关包。' })
      );
      return;
    }
    ...
    navigation.navigate('SingaporeEntryPackPreview', {
      ...
      entryPackData: {
        ...
        sgacSubmission: completionData?.sgacSubmission || null,
      },
    });
  };
      ...
      <View style={styles.currentStepContainer}>
        <View style={styles.stepHeader}>
          <View style={styles.stepCategoryBadge}>
            <Text style={styles.stepCategoryText}>{currentStep.category}</Text>
          </View>
          <Text style={styles.stepProgress}>
            {currentStepIndex + 1} / {SINGAPORE_ENTRY_STEPS.length}
          </Text>
        </View>
        ...
        {/* Show entry pack button on immigration step */}
        {currentStep.id === 'immigration' && (
          <View style={styles.entryPackCompactContainer}>
            ...
            <Text style={styles.entryPackCompactHint}>
              {t('singapore.entryGuide.entryPackHintShort', {
                defaultValue: '护照、SGAC二维码与资金凭证一键展示给移民官。',
              })}
            </Text>
          </View>
        )}
  ...
  const renderNavigationButtons = () => {
    ...
    const canGoNext = currentStepIndex < SINGAPORE_ENTRY_STEPS.length - 1;
    const isLastStep = currentStepIndex === SINGAPORE_ENTRY_STEPS.length - 1;
      ...
      <View style={styles.navigationContainer}>
        ...
        {isLastStep ? (
          <TouchableOpacity
            ...
            onPress={() => Alert.alert('恭喜！', '您已完成新加坡入境指引！')}
          >
