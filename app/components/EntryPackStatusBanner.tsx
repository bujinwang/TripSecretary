...
export interface EntryPackStatusBannerProps {
...
const STATUS_MAP: Record<EntryPackStatus, StatusConfig> = {
  submitted: {
  ...
  superseded: {
    ...
    description:
      'ข้อมูลถูกแก้ไข ต้องส่งแบบฟอร์มเข้าเมืองใหม่ / Information changed, please resubmit entry card',
  },
  expired: {
...
const EntryInfoStatusBanner: React.FC<EntryPackStatusBannerProps> = ({
  status,
  submissionDate,
  arrivalDate,
  isReadOnly = false,
}) => {
  const statusConfig = STATUS_MAP[status] ?? STATUS_MAP.in_progress;

  return (
    <View style={[styles.container, { backgroundColor: statusConfig.backgroundColor }]}>
      <View style={styles.header}>
        <Text style={[styles.icon, { color: statusConfig.textColor }]}>{statusConfig.icon}</Text>
        <View style={styles.titleContainer}>
          <Text style={[styles.title, { color: statusConfig.textColor }]}>{statusConfig.title}</Text>
      ...
      <View style={styles.dateContainer}>
        {submissionDate ? (
          <View style={styles.dateRow}>
            <Text style={[styles.dateLabel, { color: statusConfig.textColor }]}>เวลาส่ง / Submission Time:</Text>
        ...
        {arrivalDate ? (
          <View style={styles.dateRow}>
            <Text style={[styles.dateLabel, { color: statusConfig.textColor }]}>วันเข้าประเทศ / Arrival Date:</Text>
            <Text style={[styles.dateValue, { color: statusConfig.textColor }]}>
